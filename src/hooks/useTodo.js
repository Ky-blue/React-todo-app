...
// todoDataオブジェクトとして全部まとめて import
import * as todoData from "../apis/todos";

// useTodo() カスタムフックを外部ファイルで利用できるようにするため export
export const useTodo = () => {

  // todoList は現在のTODOの状態
  // setTodoList は現在の todoList の状態を更新する関数
  // todoList の初期値は空の配列
  const [todoList, setTodoList] = useState([]);

  useEffect(() => {

    // useEffect() を利用することでコンポーネントのマウント後
    // またはアンマウント後に処理を実行する
    // モックサーバーからTODOデータを取得する getAllTodosData() を実行
    todoData.getAllTodosData().then((todo) => {

      // モックサーバーからTODOデータを取得後、取得したTODOデータを反転させることで
      // TODOを追加した順に上から表示させることができる
      // Array.reverse() とスプレッド構文[ES2015]を併用することで並び替えることで
      // 元の配列を変更することなく新しい配列を作成できる
  ...
  const toggleTodoListItemStatus = (id, done) => {
    ...
    // find() は配列から条件に遭う値を見つけて、最初に true になった要素の値を返し、
    // 要素を見つけた時点で処理を停止する
    // done(完了/未完了)の状態を反転させたい todoListItem の id を見つけ、
    // 条件に一致する todoItem を返す
    const todoItem = todoList.find((item) => item.id === id);

    // 現在の todoList の中から条件に一致した要素である todoItem のdone(完了/未完了)を反転
  ...
  // 新規TODOを追加する addTodoListItem 関数の宣言
  ...
  // TODOを削除する deleteTodoListItem 関数の宣言
  const deleteTodoListItem = (id) => {
    ...
    todoData.deleteTodoData(id).then((deleteListItemId) => {
      ...
      // todoList の状態(state)を指定された id のTODOが削除された状態に更新
      setTodoList(newTodoList);
